...
class TestUserRegistration(APITestCase):
    ...
    def test_register_user_success(self):
        ...
        self.assertTrue(User.objects.filter(email='test000@example.com').exists()) #verify database
        self.assertEqual(response.data["message"], "User registered successfully!")

    def test_register_user_duplicate_email(self):
        # Create a user with the same username as above
        User.objects.create_user(email='test000@example.com', password="password123")

        # Try registering a new user with the same email
        data = {
            "email": "test000@example.com",
...
class TestUserLogin(APITestCase):   
    def setUp(self):
        ## create a user
        self.user = User.objects.create_user(email="test001@example.com", password="password123")
...
class TestUserProfile(APITestCase):
    def setUp(self):
        ...
        ## create a user
        self.user = User.objects.create_user(email="test001@example.com", password="password123", bio="a test bio", skills="cooking, tutoring")
